...
export function MetricRadar({ metrics, abbrev, labels, color = '#FFD700', accent = '#FF008C', size = 300, lang = 'en' }: Props) {
  ...
  const depth = size * 0.06; // extrusion thickness
          ...
          <svg viewBox={`${-pad} ${-pad} ${size + 2 * pad} ${size + 2 * pad}`} width="100%" aria-hidden style={{ display: 'block', width: '100%', height: 'auto' }}>
            <defs>
              ...
              <linearGradient id="mr-wall" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={hexA(color, 0.6)} />
            ...
            {/* Extruded data volume (scales in) */}
            <motion.g
            ...
            >
              ...
              {walls.map((w, i) => (
                <polygon key={i} points={w} fill="url(#mr-wall)" stroke={hexA(color, 0.25)} strokeWidth={0.5} />
              ))}
