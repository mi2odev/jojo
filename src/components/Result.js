          ...
          <div className="mb-8">
            ...
            >
              <img
                ...
                onLoad={() => console.log(`Main image loaded: ${character.image}`)}
                onError={(e) => {
                  console.error(`Main image failed to load: ${character.image}`);
  ...
  <div className="container mx-auto px-4 py-4 flex-shrink-0 text-center relative z-20">
        <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">
          ...
          <button
            onClick={() => {
              const text = lang === 'ar'
                ? `حصلت على ${character.name} في اختبار جوجو للشخصية! ${character.description}`
                : `I got ${character.name} in the JoJo Personality Test! ${character.description}`;
              if (navigator.share) {
                navigator.share({
                  title: lang === 'ar' ? 'نتيجة اختبار جوجو للشخصية' : 'JoJo Personality Test Result',
                  text,
          ...
          >
            <div className="relative bg-slate-950/60 flex items-center justify-center p-4">
              <img
                ...
                onLoad={() => console.log(`Modal image loaded: ${selectedImage.src}`)}
                onError={(e) => console.error(`Modal image failed: ${selectedImage.src}`)}
              />
              <button
                onClick={closeImageModal}
